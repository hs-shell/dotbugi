...
::: details 자동 재생을 사용하면 출석이 불인정되지 않나요?
아니요. 돋부기는 **배속이나 재생 시간 조작 없이 원래 속도(1배속)로 정상 재생**합니다. 외부 배속 프로그램과 달리 영상 속도를 변경하지 않으므로, LMS에 기록되는 재생시간과 학습시간이 동일합니다. 출석 인정 기준(재생시간 90% 이상)을 정상적으로 충족하며, 사용자가 직접 재생 버튼을 누르는 것과 동일하게 동작합니다.
:::

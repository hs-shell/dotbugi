...
describe('mergeVodWithAttendance', () => {
  ...
  it('빈 attendanceList이면 모든 VOD 누락', () => {
    const vodList = [{ week: 1, subject: '1주차', title: '강의1', url: '/v1', range: null, length: '30:00' }];
  ...
  it('같은 title이지만 다른 week이면 매칭 안 됨', () => {
    const vodList = [{ week: 1, subject: '1주차', title: '강의1', url: '/v1', range: null, length: '30:00' }];
    const attendanceList = [{ title: '강의1', isAttendance: 'O', weeklyAttendance: 'O', week: 2 }];
    const result = mergeVodWithAttendance(baseCourse, vodList, attendanceList);
    expect(result).toEqual([]);
  });

  it('같은 week이지만 다른 title이면 매칭 안 됨', () => {
    const vodList = [{ week: 1, subject: '1주차', title: '강의A', url: '/v1', range: null, length: '30:00' }];
    const attendanceList = [{ title: '강의B', isAttendance: 'O', weeklyAttendance: 'O', week: 1 }];
    const result = mergeVodWithAttendance(baseCourse, vodList, attendanceList);
    expect(result).toEqual([]);
  });
  ...
  it('course 정보가 결과에 포함됨', () => {
    const vodList = [{ week: 1, subject: '1주차', title: '강의1', url: '/v1', range: null, length: '30:00' }];
    const attendanceList = [{ title: '강의1', isAttendance: 'O', weeklyAttendance: 'O', week: 1 }];
    const result = mergeVodWithAttendance(baseCourse, vodList, attendanceList);
...
describe('mergeDueDateItems', () => {
  ...
  it('item의 필드가 course 필드와 겹치면 item이 우선', () => {
    ...
    const items = [
      { title: '과제1', dueDate: '2024-06-30', courseTitle: '재정의됨' } as {
        title: string;
        dueDate: string;
        courseTitle: string;
      },
...
describe('mergeVodWithAttendance - 추가 edge cases', () => {
  ...
  it('title에 특수문자가 포함된 경우 정확한 매칭', () => {
    const vodList = [{ week: 1, subject: '1주차', title: '강의-1(A)', url: '/v1', range: null, length: '30:00' }];
    const attendanceList = [{ title: '강의-1(A)', isAttendance: 'O', weeklyAttendance: 'O', week: 1 }];
    const result = mergeVodWithAttendance(baseCourse, vodList, attendanceList);
    expect(result).toHaveLength(1);
  });

...
- **`release.yaml`** — on PR merge to `main` (also `workflow_dispatch` / `repository_dispatch`). PR labels decide the bump (`🔖 patch` default / `🔖 minor` / `🔖 major`) and channel (`🚀 beta` → prerelease, else stable). A GitHub **Environment** approval gate (`beta` / `stable`, required reviewers) pauses the release until approved. Stable bumps `package.json`/README and pushes to `main`; beta tags only (`-beta.N`, no `main` commit). Both create a GitHub Release. Chrome Web Store auto-publish is stubbed behind `vars.ENABLE_STORE_PUBLISH`.
  - The push to protected `main` uses a **GitHub App** short-lived token (`actions/create-github-app-token`, secrets `APP_ID`/`APP_PRIVATE_KEY` scoped to the `beta`/`stable` environments; the App is on the branch-protection bypass list). No long-lived PAT.

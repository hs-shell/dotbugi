...
- **`discord-notify.yml`** — issue opened / PR opened·merged / CI failure → Discord (`DISCORD_WEBHOOK_URL`). Release notifications are sent inline from `release-please.yml` (stable=green) and `release-beta.yml` (beta=yellow), not from here.
